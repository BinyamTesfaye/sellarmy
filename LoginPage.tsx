...
import { Mail, Lock, Store, Tag, AlertCircle, Loader2 } from 'lucide-react';
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isRegistering) {
        ...
        // In demo mode, we can log in immediately
        ...
        // Real Supabase: create the profile manually if no trigger
        if (data.user) {
        ...
        // Immediately navigate to avoid race conditions with App.tsx state
        onLoginSuccess();
        navigate(role === 'SELLER' ? '/seller' : '/reseller');
      }
    } catch (err: any) {
      ...
      <div className="w-full max-w-md bg-neutral-900 border border-white/10 p-8 rounded-3xl shadow-2xl relative overflow-hidden">
        ...
        <form onSubmit={handleAuth} className="space-y-4">
          ...
          >
            {loading ? (
              ...
              <span>{isRegistering ? 'Register & Start' : 'Sign In Now'}</span>
            )}
          </button>
        </form>

        {!isSupabaseConfigured && (
          <p className="mt-4 text-[10px] text-center text-emerald-500/50 uppercase tracking-widest font-bold">
            Demo mode active: Use any email and password
          </p>
        )}
        ...
        <p className="text-center mt-6 text-sm text-neutral-500">
          ...
          <button 
            ...
            className="text-emerald-500 cursor-pointer hover:underline font-medium"
          >
